$(document).ready(() => {
  ...
  let quoteElements = () => {
    ...
    const author = (
      <p class="font-weight-bold my-auto text-right">
      - {authorString}
      </p>
    );

    const tweet = (
      <a id="tweet-quote" class="btn bg-color" href={tweetString} target="_blank"><i id="twitter-icon" class="fab fa-twitter"></i></a>
    );

    const tumblr = (
      <a id="tumblr-quote" class="btn bg-color" href={tumblrString} target="_blank"><i id="tumblr-icon" class="fab fa-tumblr"></i></a>
    );
  ...
  let newQuote = (
    <button id="new-quote" type="button" class="btn rounded bg-color" onClick={quoteElements}>
      New Quote
    </button>
  );
